...
class OrdersService
{
    ...
    protected function __initOrder( $fields, $paymentStatus, $order )
    {
        ...
        ) ?: ( $order->discount_type === 'percentage' ? $fields['discount'] /* lcabornay $this->computeOrderDiscount( $order, $fields ) */ : 0 );
        $order->discount_manager =  $fields['discount_manager'] ?? null ; // LCABORNAY
        $order->service_charge =  $fields['service_charge'] ?? 0 ; // LCABORNAY
        $order->vat_exempt =  $fields['vat_exempt'] ?? 0 ; // LCABORNAY
        $order->vat_exempt_sales =  $fields['vat_exempt_sales'] ?? 0 ; // LCABORNAY
    ...
    public function refreshOrder( Order $order )
    {
        ...
        // $order->discount = $this->computeOrderDiscount( $order ); //LCABORNAY
        // $order->total = Currency::define( $order->subtotal )
