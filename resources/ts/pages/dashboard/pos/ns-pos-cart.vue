    ...
    <div class="rounded shadow ns-tab-item flex-auto flex overflow-hidden">
      <div class="cart-table flex flex-auto flex-col overflow-hidden">
            ...
            <div class="flex flex-wrap">
              <div class="ns-button">
                ...
                >
                  ...
                  <span class="ml-1 hidden md:inline-block">{{
                    __("Comments")
                  }}</span>
                </button>
              </div>
              ...
              <div class="ns-button">
                ...
                >
                  ...
                  <span
                    v-if="order.taxes && order.taxes.length > 0"
              ...
              <div
                class="ns-button"
                v-if="options.ns_pos_quick_product === 'yes'"
              >
                <button
                  @click="openAddQuickProduct()"
                  class="w-full h-10 px-3 outline-none flex items-center"
                >
        ...
        >
          ...
          >
              ...
              <div class="flex justify-between product-details mb-1">
                ...
                <div class="-mx-1 flex product-options">
                  ...
                  <div
                    class="px-1"
                    v-if="
                      options.ns_pos_allow_wholesale_price &&
                    ...
                    <a
                      :class="
                        product.mode === 'wholesale'
                          ? 'text-success-secondary border-success-secondary'
                          : 'border-info-primary'
                      "
                ...
                <div class="-mx-1 flex flex-wrap">
                  <div class="px-1 w-1/2 md:w-auto mb-1">
                    <a
                      ...
                      :class="
                        product.mode === 'wholesale'
                          ? 'text-success-secondary hover:text-success-secondary border-success-secondary'
                          : 'border-info-primary'
                      "
                      class="cursor-pointer outline-none border-dashed py-1 border-b text-sm"
                      >{{ __("Price") }} :
                      {{ nsCurrency(product.unit_price) }}</a
                    >
                  </div>
                  <div class="px-1 w-1/2 md:w-auto mb-1">
                    <a
                  ...
                  <div class="px-1 w-1/2 md:w-auto mb-1 lg:hidden">
                    <span
                      class="cursor-pointer outline-none border-dashed py-1 border-b border-info-primary text-sm"
                      >{{ __("Total :") }}
                      {{ nsCurrency(product.total_price) }}</span
                    >
                  </div>
                </div>
              </div>
            ...
            >
              <span
                v-if="allowQuantityModification(product)"
                class="border-b border-dashed border-info-primary p-2"
        ...
        <div id="cart-products-summary" class="flex">
          <table
            class="table ns-table w-full text-sm"
            v-if="visibleSection === 'both'"
          >
            <tr>
            ...
            <tr>
              ...
              <td width="200" class="border p-2">
                ...
                >
                <span v-if="order.discount_type === 'flat'"
                  >({{ __("Flat") }})</span
                >
              </td>
              <td width="200" class="border p-2 text-right">
            ...
            <tr v-if="order.type && order.type.identifier === 'delivery'">
              ...
              <td width="200" class="border p-2 text-right">
                {{ nsCurrency(order.shipping) }}
              </td>
            </tr>
            <tr class="success">
              <td width="200" class="border p-2">
                <template
                  v-if="order && options.ns_pos_tax_type === 'exclusive'"
                >
                  <a
                    v-if="options.ns_pos_price_with_tax === 'yes'"
                    @click="openTaxSummary()"
                    class="cursor-pointer outline-none border-dashed py-1 border-b border-info-primary text-sm"
                    >{{ __("Tax Included") }}:
                    {{
                      nsCurrency(
                        order.total_tax_value + order.products_tax_value
                      )
                    }}</a
                  >
                  <a
                    v-else-if="options.ns_pos_price_with_tax === 'no'"
                    @click="openTaxSummary()"
                    class="cursor-pointer outline-none border-dashed py-1 border-b border-info-primary text-sm"
                    >{{ __("Tax") }}: {{ nsCurrency(order.total_tax_value) }}</a
                  >
                </template>
                <template
                  v-else-if="order && options.ns_pos_tax_type === 'inclusive'"
                >
                  <a
                    v-if="options.ns_pos_price_with_tax === 'yes'"
                    @click="openTaxSummary()"
                    ...
                    {{
                      nsCurrency(
                        order.total_tax_value +
                          (order.products_exclusive_tax_value +
                            order.products_inclusive_tax_value)
                      )
                    }}</a
                  >
                  <a
              ...
              <td width="200" class="border p-2">
                ...
                >
                  <a
                    ...
                    class="cursor-pointer outline-none border-dashed py-1 border-b border-info-primary text-sm"
                    >{{ __("Tax Included") }}:
                    {{ nsCurrency(order.total_tax_value) }}</a
                  >
                  <a
                    v-else-if="options.ns_pos_price_with_tax === 'no'"
                    @click="openTaxSummary()"
        ...
        >
          <template
            v-for="button of new Array(4).fill()"
            v-if="Object.keys(cartButtons).length === 0"
          >
            <div
              :class="takeRandomClass()"
...
export default {
  ...
  mounted() {
    ...
    for (let shortcut in nsShortcuts) {
      if (["ns_pos_keyboard_shipping"].includes(shortcut)) {
        nsHotPress
      ...
      if (["ns_pos_keyboard_note"].includes(shortcut)) {
        nsHotPress
  ...
  methods: {
    ...
    async changeProductPrice(product) {
      if (!this.settings.edit_purchase_price) {
        return nsSnackBar
          .error(__(`You don't have the right to edit the purchase price.`))
          .subscribe();
      }

      if (product.product_type === "dynamic") {
        return nsSnackBar
          .error(__("Dynamic product can't have their price updated."))
          .subscribe();
      }
      ...
      if (this.settings.unit_price_editable) {
        try {
          ...
          return nsSnackBar
            .success(__("The product price has been updated."))
            .subscribe();
        } catch (exception) {
          if (exception !== false) {
        ...
        return nsSnackBar
          .error(__("The editable price feature is disabled."))
          .subscribe();
      }
    },
    ...
    async defineOrderSettings() {
      if (!this.settings.edit_settings) {
        return nsSnackBar
          .error(__("You're not allowed to edit the order settings."))
          .subscribe();
      }
    ...
    async openDiscountPopup(reference, type) {
      if (!this.settings.products_discount && type === "product") {
        return nsSnackBar
          .error(__(`You're not allowed to add a discount on the product.`))
          .subscribe();
      }

      if (!this.settings.cart_discount && type === "cart") {
        return nsSnackBar
          .error(__(`You're not allowed to add a discount on the cart.`))
          .subscribe();
      }
      ...
      try {
        const promise = await new Promise((resolve, reject) => {
          Popup.show(
            ...
            {
              popupClass:
                "bg-white h:2/3 shadow-lg xl:w-1/4 lg:w-2/5 md:w-2/3 w-full",
            }
          );
        });
      } catch (exception) {
    ...
    toggleMode(product, index) {
      if (!this.options.ns_pos_allow_wholesale_price) {
        return nsSnackBar
          .error(
            __(
              "Unable to change the price mode. This feature has been disabled."
            )
          )
          .subscribe();
      }
