...
// NexoPOS and Gastro might be using different nsHooks object.
// using declare const nsHooks, will use the global object made available by the boostrap.ts file.
// this was the issue. Let's nof fix the other problem.
        ...
        const productTotal = products
          ...
          .map((product) => {
            if (usePriceWithTax === "yes") {
              if (
                ( order.discount_code === "SC" || order.discount_code === "PWD") &&
 ...
 order.sc_vatable = (productScVatable.length > 0) ?  productScVatable.reduce((b,a) => b+a) : 0 ;
    ...
    async addToCart(product) {
        ...
        let cartProduct: OrderProduct = {
            ...
            discount: 0,
            discount_percentage: 0,
            is_senior_disc: product.is_senior_disc, //LCABORNAY
