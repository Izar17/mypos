...
import { nsEvent, nsHttpClient, nsNotice, nsSnackBar } from "./bootstrap";
 ...
 */
const nsPosDashboardButton = window.nsPosDashboardButton = defineAsyncComponent(() => import('./pages/dashboard/pos/header-buttons/ns-pos-dashboard-button.vue'));
const nsPosPendingOrderButton = window.nsPosPendingOrderButton = defineAsyncComponent(() => import('./pages/dashboard/pos/header-buttons/ns-pos-' + 'pending-orders' + '-button.vue'));
const nsPosOrderTypeButton = window.nsPosOrderTypeButton = defineAsyncComponent(() => import('./pages/dashboard/pos/header-buttons/ns-pos-' + 'order-type' + '-button.vue'));
const nsPosCustomersButton = window.nsPosCustomersButton = defineAsyncComponent(() => import('./pages/dashboard/pos/header-buttons/ns-pos-' + 'customers' + '-button.vue'));
const nsPosResetButton = window.nsPosResetButton = defineAsyncComponent(() => import('./pages/dashboard/pos/header-buttons/ns-pos-' + 'reset' + '-button.vue'));
const nsPosCashRegister = window.nsPosCashRegister = defineAsyncComponent(() => import('./pages/dashboard/pos/header-buttons/ns-pos-' + 'registers' + '-button.vue'));
const nsAlertPopup = window.nsAlertPopup = defineAsyncComponent(() => import('./popups/ns-' + 'alert' + '-popup.vue'));
const nsConfirmPopup = window.nsConfirmPopup = defineAsyncComponent(() => import('./popups/ns-pos-' + 'confirm' + '-popup.vue'));
const nsPOSLoadingPopup = window.nsPOSLoadingPopup = defineAsyncComponent(() => import('./popups/ns-pos-' + 'loading' + '-popup.vue'));
const nsPromptPopup = window.nsPromptPopup = defineAsyncComponent(() => import('./popups/ns-' + 'prompt' + '-popup.vue'));
const nsLayawayPopup = window.nsLayawayPopup = defineAsyncComponent(() => import('./popups/ns-pos-' + 'layaway' + '-popup.vue'));
const nsPosShippingPopup = window.nsPosShippingPopup = defineAsyncComponent(() => import('./popups/ns-pos-' + 'shipping' + '-popup.vue'));
...
export class POS {
    ...
    _settings;
    _types;
    _orderTypeProcessQueue = [];
    _paymentsType;
    _order;
    _screen;
    _holdPopupEnabled = true;
    _initialQueue = [];
    _options;
    _responsive = new Responsive;
    _visibleSection;
    _isSubmitting = false;
    _processingAddQueue = false;
    ...
    initialize() {
        this._userPermissions = new BehaviorSubject([]);
        ...
        this._cartButtons = new BehaviorSubject({});
        ...
        this.initialQueue.push(() => new Promise((resolve, reject) => {
            ...
            if (options.ns_customers_default !== false) {
                nsHttpClient.get(`/api/customers/${options.ns_customers_default}`)
                    ...
                    error: (error) => {
                        nsNotice
                            .error(__('An error has occured'), __('Unable to select the default customer. Looks like the customer no longer exists. Consider changing the default customer on the settings.'), {
                            actions: {
                                readMore: {
                                    label: __('Read More'),
                                    onClick: (instance) => {
    ...
    computeOrderTaxGroup(order, tax) {
        const summarizedRates = tax.taxes.map(tax => parseFloat(tax.rate)).reduce((b, a) => b + a);
        ...
        tax.taxes = tax.taxes.map(_tax => {
            const currentPercentage = math.chain(math.chain(_tax.rate).divide(summarizedRates).done()).multiply(100).done();
            return {
                id: _tax.id,
                name: _tax.name,
                rate: parseFloat(_tax.rate),
                tax_value: math.chain(math.chain(currentVatValue).multiply(currentPercentage).done()).divide(100).done()
            };
        });
    ...
    async refreshCart() {
        ...
        console.log('products', products); // lcabornay
        console.log('order', order); // lcabornay
        console.log('order.discount_code', order.discount_code);
        ...
        // lcabornay
        const productVatExemptSales = products.filter((product) => product.product_type !== "dynamic")
            .map((product) => {
            if (usePriceWithTax === "yes") {
                if ((order.discount_code === "SC" || order.discount_code === "PWD") &&
                    (product.is_senior_disc === true || product.product?.is_senior_disc === true)) {
                    console.log("inside productVatExemptSales", product.total_price_without_tax);
                    return product.total_price_without_tax;
                }
                else {
                    return 0;
                }
        ...
        const productVatExempt = products.filter((product) => product.product_type !== "dynamic")
            .map((product) => {
            if (usePriceWithTax === "yes") {
                if ((order.discount_code === "SC" || order.discount_code === "PWD") &&
                    (product.is_senior_disc === true || product.product?.is_senior_disc === true)) {
                    return product.total_price_with_tax - product.total_price_without_tax;
                }
                else {
                    return 0;
                }
        ...
        const productScVatable = products.filter((product) => product.product_type !== "dynamic")
            .map((product) => {
            if (usePriceWithTax === "yes") {
                if ((order.discount_code === "SC" || order.discount_code === "PWD") &&
                    (product.is_senior_disc === false || product.product?.is_senior_disc === false)) {
                    return product.total_price_with_tax;
                }
                else {
                    return 0;
                }
        ...
        console.log('product vat exempt sales', productVatExemptSales);
        console.log('product vat exempt', productVatExempt);
        console.log('product vatable SC', productScVatable);
        order.vat_exempt_sales = (productVatExemptSales.length > 0) ? productVatExemptSales.reduce((b, a) => b + a) : 0;
        order.vat_exempt = (productVatExempt.length > 0) ? productVatExempt.reduce((b, a) => b + a) : 0;
        order.sc_vatable = (productScVatable.length > 0) ? productScVatable.reduce((b, a) => b + a) : 0;
        ...
        if (taxType === 'exclusive') {
            ...
            (order.subtotal + (order.shipping || 0)) - (order.discount + order.total_coupons + order.vat_exempt) // LCABORNAY
        ...
        if (order.discount_code === "FREE") {
            ...
            order.service_charge = (order.subtotal * .10); // lcabornay
        }
        console.log('service charge', order.service_charge);
        console.log('discount', order.discount);
        order.total = +(order.total + order.service_charge).toFixed(ns.currency.ns_currency_precision); // lcabornay
    ...
    async addToCart(product) {
        ...
        let cartProduct = {
            ...
            discount: 0,
            discount_percentage: 0,
            is_senior_disc: product.is_senior_disc,
        ...
        if (cartProduct.product_id !== 0) {
            for (let index in this.addToCartQueue) {
                ...
                try {
                    ...
                    const result = (await promiseInstance.run(productData));
                    /**
    ...
    resumeRemovingProductUsingIndex(index, products) {
    ...
    computeProduct(product) {
        ...
        if (['flat', 'percentage'].includes(product.discount_type)) {
            if (product.discount_type === 'percentage') {
                product.discount = math.chain(math.chain(math.chain(product.unit_price).multiply(product.discount_percentage).done()).divide(100).done()).multiply(product.quantity).done();
                discount_without_tax = math.chain(math.chain(math.chain(price_without_tax).multiply(product.discount_percentage).done()).divide(100).done()).multiply(product.quantity).done();
                discount_with_tax = math.chain(math.chain(math.chain(price_with_tax).multiply(product.discount_percentage).done()).divide(100).done()).multiply(product.quantity).done();
            }
            else {
        ...
        product.total_price = math.chain(math.chain(product.unit_price).multiply(product.quantity).done()).subtract(product.discount).done();
        product.total_price_with_tax = math.chain(math.chain(price_with_tax).multiply(product.quantity).done()).subtract(discount_with_tax).done();
        product.total_price_without_tax = math.chain(math.chain(price_without_tax).multiply(product.quantity).done()).subtract(discount_without_tax).done();
