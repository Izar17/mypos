export class NsHotPress {
    ...
    processEvent(event) {
        for (let index in this.listeners) {
             ...
             */
            const config = this.listeners[index].getConfig();
            if (config.hidden.length > 0) {
                const hiddenPassed = config.hidden.filter((element) => {
            ...
            if (config.visible.length > 0) {
                const visiblePassed = config.visible.filter((element) => {
    ...
    processSingleAction({ action, callback }) {
        ...
        combinaisons.forEach(combinaison => {
            switch (combinaison) {
                case 'ctrl':
                    combinableKeys.ctrlKey = true;
                    break;
                case 'alt':
                    combinableKeys.altKey = true;
                    break;
                case 'shift':
                    combinableKeys.shiftKey = true;
                    break;
            }
        });
...
//# sourceMappingURL=ns-hotpress.js.map