...
export class FloatingNotice {
    ...
    show(title, description, options = { duration: 3000, type: 'info' }) {
    ...
    error(title, description, options = { duration: 3000, type: 'error' }) {
        return this.show(title, description, { ...options, ...{ type: 'error' } });
    }
    success(title, description, options = { duration: 3000, type: 'success' }) {
        return this.show(title, description, { ...options, ...{ type: 'success' } });
    }
    info(title, description, options = { duration: 3000, type: 'info' }) {
        return this.show(title, description, { ...options, ...{ type: 'info' } });
    }
    warning(title, description, options = { duration: 3000, type: 'warning' }) {
    ...
    __createSnack({ title, description, options }) {
        ...
        if (options.actions !== undefined && Object.values(options.actions).length > 0) {
            for (let key in options.actions) {
                ...
                buttonsWrapper.appendChild(buttonDom.querySelector('.ns-button'));
            }
        }
...
//# sourceMappingURL=floating-notice.js.map