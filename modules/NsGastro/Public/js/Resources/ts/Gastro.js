...
class Gastro {
            ...
            orderLines.forEach(line => {
                if (line.dom && !line.dom.querySelector('.gastro-controls')) {
                    ...
                    button.addEventListener('click', async () => {
                        ...
                        switch (promise) {
                            case 'split':
                                Popup.show(gastroSplitOrder, { order: line.order });
                                break;
                            case 'merge':
                                this.selectOrderForMerging(line.order);
                                break;
                        }
                    });
    ...
    setAddButtonsVisibility(status) {
        if (status === 'visible') {
        ...
        if (product.modifiersGroups && product.modifiersGroups.length > 0) {
            ...
            modifierContainer.addEventListener('click', async function () {
                ...
                try {
                    ...
                    const response = (await (modifierPromise.run(product)));
...
//# sourceMappingURL=Gastro.js.map