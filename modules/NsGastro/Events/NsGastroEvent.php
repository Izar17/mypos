...
class NsGastroEvent
{
    ...
    public static function freedTableIfNecessary( OrderAfterDeletedEvent $event )
    {
        $table = Table::find( $event->order->table_id );

        if ( $table instanceof Table ) {
            self::setTableAsFree( $table );
        }
    }
    ...
    public static function saveTable(OrderAfterCreatedEvent $event)
    {
        if (isset($event->fields['table']) && isset($event->fields['table']['selected']) && $event->fields['table']['selected'] === true) {
            $table = Table::findOrFail( $event->fields['table']['id']);
            ...
            $event->order->gastro_table_session_id = $session instanceof TableSession ? $session->id : null;
            $event->order->save();
        }
    }
    ...
    private static function startTableSession( Table $table ): TableSession | null
    {
        ...
        if ((bool) ns()->option->get('ns_gastro_enable_table_sessions', false)) {
            ...
            /**
             * As we've create an order on a table,
             * we'll mark that as busy if the order is not yet paid.
             */
            $table->busy = true;
            $table->save();
    ...
    public static function updateOrderStatus(OrderAfterUpdatedEvent $event)
    {
        ...
        $pending = $event->newOrder->products
            ->filter(fn ($product) => $product->cooking_status === GastroOrderProduct::COOKING_PENDING)
            ->count();

        $ongoing = $event->newOrder->products
            ->filter(fn ($product) => $product->cooking_status === GastroOrderProduct::COOKING_ONGOING)
            ->count();

        $ready = $event->newOrder->products
            ->filter(fn ($product) => $product->cooking_status === GastroOrderProduct::COOKING_READY)
            ->count();

        $totalProducts = $event->newOrder->products->count();

        if (! in_array($event->newOrder->gastro_order_status, [GastroOrder::COOKING_REQUESTED, GastroOrder::COOKING_PROCESSED])) {
            if ($pending === $totalProducts || $pending > 0 && $ongoing === 0) {
                $event->newOrder->gastro_order_status = GastroOrder::COOKING_PENDING;
            } elseif ($ongoing > 0) {
                $event->newOrder->gastro_order_status = GastroOrder::COOKING_ONGOING;
            } elseif ($ready === $totalProducts) {
                $event->newOrder->gastro_order_status = GastroOrder::COOKING_READY;
            }
        }
    ...
    /**
     * Will mark a table as freed
     *
     * @param  Order  $order
     * @return void
     */
    public static function setTableAsFree( Table $table )
    {
        /**
        ...
        if (
            ...
            $session = $table->sessions()->where('active', true)->first();
            ...
            if ($totalPaid === $session->orders()->count()) {
                ...
                if ((bool) ns()->option->get('ns_gastro_enable_table_sessions', false)) {
                    ...
                    $session->session_minutes_duration = (int) Carbon::parse($session->session_starts)
                        ->diffInMinutes($session->session_ends);
                    $session->save();
                }
            }
        } else {
            $totalPaid  =   Order::where( 'table_id', $table->id )
                ->where( 'payment_status', Order::PAYMENT_PAID )
                ->count();

            $totalOrders    =   Order::where( 'table_id', $table->id )->count();

            if ( $totalPaid === $totalOrders ) {
                $table->busy    =   false;
                $table->save();
            }
        }
    ...
    // public function freeTableIfNecessary(OrderAfterPaymentStatusChangedEvent $event) //lcabornay
    public function freeTableIfNecessary(OrderAfterUpdatedEvent $event)
    {
        // $table = Table::find($event->order->table_id);
        // dd($event->prevOrder,  $event->newOrder , $event->newOrder->table_id);
        $table = Table::find($event->newOrder->table_id);
        if ( 
            $table instanceof Table && 
            // $event->previous === Order::PAYMENT_HOLD && 
            $event->prevOrder->payment_status === Order::PAYMENT_HOLD && 
            // $event->new === Order::PAYMENT_PAID && 
            $event->newOrder->payment_status === Order::PAYMENT_PAID && 
            ( bool ) ns()->option->get( 'ns_gastro_freed_table_with_payment' ) ) {
            self::setTableAsFree($table);
        }
    }
    ...
    public function setModifierGroups( ProductAfterCreatedEvent | ProductAfterUpdatedEvent $event )
    {
        /**
         * @var ProductModifiersService
         */
        $service    =   app()->make( ProductModifiersService::class );
        $service->setProductModifierGroup( $event->product );
    }

    public function removeModifierGroups( ProductBeforeDeleteEvent $event )
    {
        ProductModifierGroup::where( 'product_id', $event->product->id )->delete();
    }
}
